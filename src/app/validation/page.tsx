...
import { validationScreens } from '@/components/ValidationScreens'
...
export default function ValidationPage() {
  ...
  const screens = validationScreens
    ...
    <div className="min-h-screen lg:h-[100dvh] lg:overflow-hidden bg-gradient-to-br from-slate-900 via-[#0f1729] to-slate-900 flex flex-col items-center">
      ...
      <div className="w-full flex flex-wrap items-center gap-x-6 gap-y-3 px-5 pt-3 pb-2">
        <div className="flex-shrink-0">
          ...
          <p className="text-slate-400 text-[11px]">5 experiments, ordered by confidence × stakes</p>
        </div>
      ...
      <div className="flex items-start justify-center gap-6 px-4 mt-1 pb-6 lg:pb-0 lg:flex-1 lg:min-h-0 w-full">
        ...
        {/* Left: stage list */}
        <div className="hidden lg:flex flex-col gap-1 w-56 pt-1 flex-shrink-0 self-center max-h-full overflow-y-auto">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 px-2">Validation Stages</p>
          {screens.map((s, i) => (
            <button key={s.id}
              onClick={() => goTo(s.id)}
