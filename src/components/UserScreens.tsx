    ...
    <div className="flex-1 overflow-y-auto px-4 pb-28">
      ...
      <div className="flex justify-between items-center mt-1 mb-4">
        <div>
          ...
          <h2 className="text-xl font-black text-slate-900">Akash Sharma 👋</h2>
        </div>
      ...
      {/* Balance Card */}
      <div className="mb-4">
        <DebitCard name="AKASH SHARMA" balance="₹48,250" balanceLabel="Available Balance" last4="4821" />
      </div>
      ...
      <button onClick={onNext}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-blue-200 text-xs leading-relaxed">Give your child their own card with daily limits and controls you set.</p>
          </div>
...
const S3_AddType: React.FC<ScreenProps> = ({ onBack, goTo }) => (
  <div className="flex flex-col h-full bg-slate-50">
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-6">
      ...
      {/* Child */}
      <button onClick={() => goTo('add-details')}
        ...
        <div className="flex-1">
          <div className="text-sm font-black text-slate-900">Child</div>
      ...
      {/* Spouse */}
      <button onClick={() => goTo('add-spouse-details')}
        className="w-full bg-white border border-slate-200 rounded-2xl p-4 flex items-center gap-4 mb-3 text-left shadow-sm">
        <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-2xl flex-shrink-0">💑</div>
        <div className="flex-1">
          <div className="text-sm font-bold text-slate-900">Spouse / Partner</div>
          <div className="text-xs text-slate-500">Supplementary card on your account</div>
        </div>
        <span className="text-slate-300 text-xl flex-shrink-0">›</span>
      </button>

      {/* Parent / Elder */}
      <button onClick={() => goTo('add-parent-details')}
        className="w-full bg-white border border-slate-200 rounded-2xl p-4 flex items-center gap-4 mb-5 text-left shadow-sm">
        <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-2xl flex-shrink-0">👴</div>
        <div className="flex-1">
          <div className="text-sm font-bold text-slate-900">Parent / Elder</div>
          <div className="text-xs text-slate-500">Supplementary card with elder-care controls</div>
        </div>
        <span className="text-slate-300 text-xl flex-shrink-0">›</span>
      </button>

      <InfoBox color="amber">
        ℹ️ <strong>How it works:</strong> Minors (under 18) get a guardian-linked account per the RBI April 2025 circular. Adults (spouse, parent) get a supplementary card on your account that you fund and control.
      </InfoBox>

      <div className="mt-5">
        <Btn onClick={() => goTo('add-details')}>Continue with Child →</Btn>
      </div>
    </div>
  </div>
)
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-6">
      ...
      <div className="mt-4 bg-white rounded-2xl p-4 flex items-center gap-3 border border-slate-100 shadow-sm mb-5">
        ...
        <div>
          ...
          <div className="text-sm font-black text-slate-900">Akash Sharma</div>
...
const S6_InviteSent: React.FC<ScreenProps> = ({ onBack, goTo }) => (
  <div className="flex flex-col h-full bg-slate-50">
    <StatusBar />
    ...
    <div className="flex-1 overflow-y-auto px-5 pb-6">
      ...
      <div className="mt-5 space-y-2.5">
        <Btn onClick={() => goTo('hub-dashboard')}>View Family Hub →</Btn>
...
// ═══ SPOUSE FLOW (supplementary card) ═══════════════════════════════════════

// Add Spouse - Details
const S_SpouseDetails: React.FC<ScreenProps> = ({ onNext, onBack }) => (
  <div className="flex flex-col h-full bg-slate-50">
    <StatusBar />
    <NavBar title="Spouse Details" onBack={onBack} step="Step 2 / 3" />

    <div className="flex-1 overflow-y-auto px-4 pb-6">
      <p className="text-xs text-slate-500 mb-5 mt-1">Adults get a supplementary card on your account.</p>

      <InputField label="Full Name" value="Rahul Sharma" />
      <InputField label="Mobile Number (for invite)" value="+91 91234 56780" />
      <InputField label="Relationship" value="Spouse / Partner" />

      <InfoBox color="blue">
        <strong>Supplementary card:</strong> Rahul rides on your Optimus account. He completes a quick KYC; you fund, monitor, and can freeze the card anytime.
      </InfoBox>

      <div className="mt-4 bg-white rounded-2xl p-4 flex items-center gap-3 border border-slate-100 shadow-sm mb-5">
        <div className="w-10 h-10 bg-blue-50 rounded-full flex items-center justify-center text-lg flex-shrink-0">👤</div>
        <div>
          <div className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider mb-0.5">Supplementary to</div>
          <div className="text-sm font-black text-slate-900">Akash Sharma</div>
          <div className="text-[10px] text-[#4361EE] font-semibold">Primary account holder</div>
        </div>
      </div>
...
// Set Spouse Limits
const S_SpouseLimits: React.FC<ScreenProps> = ({ onNext, onBack }) => (
  <div className="flex flex-col h-full bg-slate-50">
    <StatusBar />
    <NavBar title="Set Limits" onBack={onBack} step="Step 3 / 3" />

    <div className="flex-1 overflow-y-auto px-4 pb-6">
      <p className="text-xs text-slate-500 mb-4 mt-1">You can change these anytime from Rahul's card.</p>

      <div className="bg-white rounded-2xl px-4 py-1 shadow-sm mb-4">
        <SectionLabel className="mt-3">Spending Limits</SectionLabel>
        <LimitBox label="Per Transaction" sub="Max single spend" value="₹25,000" />
        <LimitBox label="Daily Limit" sub="Total per day" value="₹25,000" />
        <LimitBox label="Monthly Limit" sub="Total per month" value="₹50,000" />
      </div>

      <div className="bg-white rounded-2xl px-4 py-1 shadow-sm mb-4">
        <SectionLabel className="mt-3">Category Controls (optional)</SectionLabel>
        <CatRow emoji="🛒" label="Groceries & Shopping" sub="Supermarkets, retail" on={true} />
        <CatRow emoji="🍔" label="Food & Dining" sub="Restaurants, delivery" on={true} />
        <CatRow emoji="✈️" label="Travel & Fuel" sub="Flights, cabs, petrol" on={true} />
        <CatRow emoji="🎰" label="Gambling & Betting" sub="Fantasy sports, casino" on={false} />
      </div>

      <InfoBox color="emerald">
        ✅ <strong>Approval rule:</strong> Transactions above ₹10,000 need your approval. The card spends from your account.
      </InfoBox>

      <div className="mt-5">
        <Btn onClick={onNext}>Send Card Invite to Rahul →</Btn>
      </div>
    </div>
  </div>
)
...
// Spouse Invite Sent
const S_SpouseInvite: React.FC<ScreenProps> = ({ onBack, goTo }) => (
  <div className="flex flex-col h-full bg-slate-50">
    <StatusBar />
    <NavBar title="" onBack={onBack} />
    ...
    <div className="flex-1 overflow-y-auto px-5 pb-6">
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl font-black text-slate-900 mb-2">Invite sent!</h2>
        <p className="text-sm text-slate-500 leading-relaxed">
          Rahul will receive an SMS invite. After a quick KYC, the supplementary card activates instantly.
        </p>
      </div>

      <DebitCard name="RAHUL SHARMA" last4="••••" label="Supplementary">
        <div className="text-xs opacity-60 mb-2">Supplementary Card · Awaiting activation</div>
      </DebitCard>
      ...
      <div className="bg-white rounded-2xl px-4 py-1 mt-4 shadow-sm">
        ...
        <div className="space-y-2.5 pb-3">
          {[
            { label: 'Card type', value: 'Supplementary' },
            { label: 'Monthly limit', value: '₹50,000' },
            { label: 'Approval above', value: '₹10,000' },
            { label: 'Funded by', value: 'Akash Sharma' },
            { label: 'Invite sent to', value: '+91 91234 56780' },
          ].map(r => (
            <div key={r.label} className="flex justify-between text-sm">
              <span className="text-slate-500">{r.label}</span>
              <span className="font-bold text-slate-900">{r.value}</span>
...
// ═══ PARENT / ELDER FLOW (supplementary card + elder care) ═══════════════════

// Add Parent - Details
const S_ParentDetails: React.FC<ScreenProps> = ({ onNext, onBack }) => (
  <div className="flex flex-col h-full bg-slate-50">
    <StatusBar />
    <NavBar title="Parent / Elder Details" onBack={onBack} step="Step 2 / 3" />

    <div className="flex-1 overflow-y-auto px-4 pb-6">
      <p className="text-xs text-slate-500 mb-5 mt-1">Set up an elder-friendly card with fraud protection.</p>

      <InputField label="Full Name" value="Suresh Sharma" />
      <InputField label="Mobile Number (for invite)" value="+91 90000 12345" />
      <InputField label="Relationship" value="Parent (Father) · Age 68" />

      <InfoBox color="amber">
        👴 <strong>Elder care:</strong> Suresh gets a supplementary card with scam-alert protection and a large-text app. You monitor spends and are alerted to any unusual activity.
      </InfoBox>
      ...
      <div className="mt-4 bg-white rounded-2xl p-4 flex items-center gap-3 border border-slate-100 shadow-sm mb-5">
        ...
        <div>
          <div className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider mb-0.5">Supplementary to</div>
          <div className="text-sm font-black text-slate-900">Akash Sharma</div>
          <div className="text-[10px] text-[#4361EE] font-semibold">Primary account holder · Caregiver</div>
        </div>
      </div>

      <Btn onClick={onNext}>Set Controls →</Btn>
    </div>
  </div>
)

// Set Parent Controls
const S_ParentControls: React.FC<ScreenProps> = ({ onNext, onBack }) => (
  <div className="flex flex-col h-full bg-slate-50">
    <StatusBar />
    <NavBar title="Set Controls" onBack={onBack} step="Step 3 / 3" />

    <div className="flex-1 overflow-y-auto px-4 pb-6">
      <p className="text-xs text-slate-500 mb-4 mt-1">Tuned for safety. You can change these anytime.</p>

      <div className="bg-white rounded-2xl px-4 py-1 shadow-sm mb-4">
        <SectionLabel className="mt-3">Spending Limits</SectionLabel>
        <LimitBox label="Per Transaction" sub="Max single spend" value="₹10,000" />
        <LimitBox label="Daily Limit" sub="Total per day" value="₹15,000" />
        <LimitBox label="Monthly Limit" sub="Total per month" value="₹40,000" />
      </div>

      <div className="bg-white rounded-2xl px-4 py-1 shadow-sm mb-4">
        <SectionLabel className="mt-3 text-[#EA580C]">Elder-care Protection</SectionLabel>
        <CatRow emoji="🛡️" label="Scam & fraud alerts" sub="Flag suspicious payments" on={true} />
        <CatRow emoji="🚫" label="Block high-risk merchants" sub="Crypto, betting, lottery" on={true} />
        <CatRow emoji="🌍" label="Block international online" sub="Foreign web payments" on={true} />
        <CatRow emoji="🔔" label="Unusual-activity alerts" sub="Real-time to you" on={true} />
      </div>

      <InfoBox color="red">
        🛡️ You will be alerted in real time to any high-risk or unusual transaction on Suresh's card. Approval is required above ₹5,000.
      </InfoBox>

      <div className="mt-5">
        <Btn onClick={onNext}>Send Card Invite to Suresh →</Btn>
      </div>
    </div>
  </div>
)
...
// Parent Invite Sent
const S_ParentInvite: React.FC<ScreenProps> = ({ onBack, goTo }) => (
  <div className="flex flex-col h-full bg-slate-50">
    <StatusBar />
    <NavBar title="" onBack={onBack} />
    ...
    <div className="flex-1 overflow-y-auto px-5 pb-6">
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl font-black text-slate-900 mb-2">Invite sent!</h2>
        <p className="text-sm text-slate-500 leading-relaxed">
          Suresh will receive an SMS invite with simple, large-text setup. The card activates after a quick KYC.
        </p>
      </div>

      <DebitCard name="SURESH SHARMA" last4="••••" label="Parent · Elder">
        <div className="text-xs opacity-60 mb-2">Supplementary Card · Awaiting activation</div>
      </DebitCard>
      ...
      <div className="bg-white rounded-2xl px-4 py-1 mt-4 shadow-sm">
        ...
        <div className="space-y-2.5 pb-3">
          {[
            { label: 'Card type', value: 'Supplementary' },
            { label: 'Scam protection', value: '✓ On' },
            { label: 'Fraud alerts', value: '✓ On' },
            { label: 'Monthly limit', value: '₹40,000' },
            { label: 'Funded by', value: 'Akash Sharma' },
          ].map(r => (
            <div key={r.label} className="flex justify-between text-sm">
              <span className="text-slate-500">{r.label}</span>
  ...
  <div className="flex flex-col h-full bg-slate-50 relative">
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-28">
      ...
      <div className="rounded-2xl p-4 mb-5 text-white"
        ...
        <div className="text-3xl font-black mb-1">₹6,540</div>
        <div className="flex gap-4 text-[11px] opacity-75">
          <span>3 members active</span>
          <span>·</span>
          <span>9 transactions</span>
        </div>
      </div>

      <SectionLabel>Your Members</SectionLabel>
      <MemberCard
        initials="A" name="Aarav Sharma" tag="Child · 13 yrs · Active"
        pct={62} left="₹760" today="₹1,240"
        color="linear-gradient(135deg, #667EEA, #764BA2)"
        onClick={() => goTo('member-detail')}
      />
      <MemberCard
        initials="R" name="Rahul Sharma" tag="Spouse · Supplementary card"
        pct={17} left="₹20,800" today="₹4,200"
        color="linear-gradient(135deg, #7C3AED, #4C1D95)"
        onClick={() => goTo('member-detail')}
      />
      <MemberCard
        initials="S" name="Suresh Sharma" tag="Parent · Elder care"
        pct={7} left="₹13,900" today="₹1,100"
      ...
      <div className="bg-white rounded-2xl px-4 shadow-sm">
        <TxnItem icon="🛒" name="BigBasket · Rahul" sub="Today · 2:10 PM · Groceries" amount="−₹2,400" />
        <TxnItem icon="🍕" name="Zomato · Aarav" sub="Today · 1:34 PM · Food" amount="−₹280" />
        <TxnItem icon="💊" name="Apollo Pharmacy · Suresh" sub="Today · 11:20 AM · Medical" amount="−₹1,100" />
...
const S8_MemberDetail: React.FC<ScreenProps> = ({ onNext, onBack, goTo }) => (
  <div className="flex flex-col h-full bg-slate-50">
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-6">
      {/* Compact card */}
      <div className="mb-4">
        <DebitCard name="AARAV SHARMA" balance="₹760" balanceLabel="remaining today of ₹2,000" last4="7291" label="Child" chip={false}>
...
export const userScreens: Screen[] = [
  ...
  // Child flow
  { id: 'add-details',          label: 'Add Child: Details',      component: S4_AddDetails },
  { id: 'set-limits',           label: 'Child: Limits',           component: S5_SetLimits },
  { id: 'invite-sent',          label: 'Child Invite Sent ✓',     component: S6_InviteSent },
  // Spouse flow
  { id: 'add-spouse-details',   label: 'Add Spouse: Details',     component: S_SpouseDetails },
  { id: 'set-spouse-limits',    label: 'Spouse: Limits',          component: S_SpouseLimits },
  { id: 'invite-sent-spouse',   label: 'Spouse Invite Sent ✓',    component: S_SpouseInvite },
  // Parent / Elder flow
  { id: 'add-parent-details',   label: 'Add Parent: Details',     component: S_ParentDetails },
  { id: 'set-parent-controls',  label: 'Parent: Controls',        component: S_ParentControls },
  { id: 'invite-sent-parent',   label: 'Parent Invite Sent ✓',    component: S_ParentInvite },
  // Hub management
